...
class Listing extends Component
{
    ...
    {
        return view('backend::livewire.configuration.term-and-use.listing');
    }
    ...
    public function save()
    {
        ...
        TermAndUseDates::updateOrCreate(
            [
                'id'                    => $this->model->id,
            ],
            [
        ...
        $this->defaultload();
        session()->flash('success', 'Term of Use Dates update successfully');

        return redirect()->route('configuration.termsanduse.index');
    }
    ...
    public function updatedModel()
    {
        ...
        if (!empty($this->model)) {
            $this->model =  TermAndUseDates::where('status', true)->first();
        }

        if (empty($this->model)) {
            $this->model = new TermAndUseDates();
        }
    }
