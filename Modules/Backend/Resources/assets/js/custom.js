$(function () {
	...
	(function () {
		...
		if (bodyhorizontal) {
			...
			$(".main-sidemenu").addClass("container-fluid")
	...
	$(".filter-button").click(function () {
		...
		if (value == "") {
			...
			$('.card-filter-category').show('1000');
		}
		else {
			...
			$(".card-filter-category").not('.' + value).hide('3000');
			$('.card-filter-category').filter('.' + value).show('3000');

		}
		spiderCardFilterCategoryColumnShowhide();
	});

	$("#search_box").on("keyup", function () {
		var value = $(this).val().toLowerCase();
		$("#cardGroupNotes .card-filter-category").filter(function () {
			$(this).toggle($(this).text().toLowerCase().indexOf(value) > -1);
			spiderCardFilterCategoryColumnShowhide();

			// $(this).find('.card-filter-category').hide('1000');
		});
	});

	function spiderCardFilterCategoryColumnShowhide() {

	}

});
function countdowntimes() {
	var livedt = new Date().toUTCString();
	// var D = livedt.getWeekDay();
	// var d = livedt.getDate();
	// var M = livedt.getMonth();
