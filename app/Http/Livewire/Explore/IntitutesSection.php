...
class IntitutesSection extends Component
{
    // use WithPagination;
    ...
    {
        $this->$filterState = Helper::NOT_NEEDED;
        $this->filterArea = Helper::NOT_NEEDED;
    ...
    public function instituteCompare($id)
    {
        $compareCount = count(Session::get('compare.institute', []));
        ...
        if ($compareCount < 3) {
            ...
            // Display a message indicating that only 3 items can be compared
            session()->flash('message', 'You can only compare up to 3 items.');
        }
