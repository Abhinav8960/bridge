...
class ForeignLanguageExams extends Component
{
    const  EXAM_ID = 3;
    ...
    {
        $this->category = Category::where('id', self::EXAM_ID)->where('status',true)->first();
        if (!empty($this->category)) {
            ...
            $this->visibility = $this->category->is_show_homepage;
        } else {
    ...
    public function render()
    {
        return view(
            ...
            [
                'exams' => Exam::where('stream_id', $this->selectedStream)->where('status', true)->paginate(10),
            ]
        );
    }
